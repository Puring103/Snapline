...
use snapline_domain::{
    ...
    AssetMetadata, Note, NoteChangePayload, SyncPayload,
};
...
async fn upload_pending_assets_from_path<A: SyncApi + Sync>(
    db_path: &Path,
    api: &A,
    token: &str,
    data_dir: &Path,
...
async fn push_pending_changes_from_path<A: SyncApi + Sync>(
    db_path: &Path,
    api: &A,
    token: &str,
    device_id: &str,
...
async fn pull_remote_changes_from_path<A: SyncApi + Sync>(
    db_path: &Path,
    api: &A,
    token: &str,
    device_id: &str,
...
async fn import_snapshot_and_assets_from_path<A: SyncApi + Sync>(
    db_path: &Path,
    api: &A,
    token: &str,
    data_dir: &Path,
...
/// 加密笔记 payload 中的 title 和 content_md 字段。
fn encrypt_note_payload(dek: &[u8; 32], payload: &NoteChangePayload) -> Result<NoteChangePayload> {
...
/// 解密从服务端接收到的笔记的 title 和 content_md 字段。
fn decrypt_note(dek: &[u8; 32], note: &Note) -> Result<Note> {
    Ok(Note {
