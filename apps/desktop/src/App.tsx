...
import { getAiConfig, logout, processAiQueue, sessionStatus, type AiConfigStatus } from './lib/native';
...
export function App() {
  ...
  if (captureType) return <Capture initialType={captureType} onClose={() => { setCaptureType(null); history.replaceState(null, '', location.pathname); }} onCreated={(item) => setItems((current) => [{ ...item, ai_status: aiConfig.configured ? item.ai_status : 'unconfigured' }, ...current.filter((value) => value.id !== item.id)])} />;
  ...
  return <main className={`app-shell ${aiOpen ? 'with-ai' : ''}`}>
    ...
    {aiOpen && <AiPanel configured={aiConfig.configured} onConfigure={() => setAiSettingsOpen(true)} onClose={() => setAiOpen(false)} />}
