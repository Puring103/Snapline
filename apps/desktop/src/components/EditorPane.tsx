...
import { Archive, ArchiveRestore, Bold, Check, Code2, Heading2, ImagePlus, Italic, Link, List, ListChecks, Mic, MoreHorizontal, Pin, Quote, Redo2, Sparkles, Trash2, Undo2, X } from 'lucide-react';
...
export function EditorPane({ item, onChange, onSave, onDelete, onRecord }: { item?: Item; onChange: (item: Item) => void; onSave: (item: Item) => Promise<void>; onDelete: (id: string) => void; onRecord: () => void }) {
  ...
  return <section className="editor-pane">
    <header className="editor-head"><div className={`save-state ${saveState}`}><i />{saveState === 'saved' ? '已保存到本地' : saveState === 'saving' ? '正在保存…' : '保存失败，正在重试'}</div><div className="editor-actions"><button className={`icon-button ${item.pinned ? 'is-active' : ''}`} title={item.pinned ? '取消收藏' : '收藏'} aria-label={item.pinned ? '取消收藏' : '收藏'} onClick={() => onChange({ ...item, pinned: !item.pinned })}><Pin size={17} /></button><button className="icon-button" title={item.archived ? '恢复' : '归档'} aria-label={item.archived ? '恢复' : '归档'} onClick={() => onChange({ ...item, archived: !item.archived })}>{item.archived ? <ArchiveRestore size={17} /> : <Archive size={17} />}</button><button className="icon-button" title="更多" aria-label="更多"><MoreHorizontal size={18} /></button></div></header>
